...
- Current modules:
  ...
  - `app/V1/Modules/Dashboard/**`
  - `app/V1/Modules/Inquiry/**`
  - `app/V1/Modules/Offer/**`
  - `app/V1/Modules/Order/**`
...
- For MVP tenant isolation, every operational company-owned model introduced after `Company` must include a `company_id` ownership boundary, API queries must scope reads/writes to the authenticated user's company, and focused feature tests must cover cross-company read/write denial before the React CRUD is considered complete.
...
- Query ordering/filtering used by API repositories must stay compatible with the sqlite test database defined in `phpunit.xml`; avoid database-specific helpers such as MySQL `FIELD(...)` unless a tested connection-specific alternative is implemented.
